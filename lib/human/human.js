...
export class BaseHuman extends THREE.Object3D {
  ...
  loadModel() {
    ...
    return new Promise((resolve, reject) => {
      ...
      if (modelUrl == "node_modules/makehuman-data/public/data/models/human_full_size.json") {
        return resolve(JSON.stringify(data_models_human_full_size))
      }
      ...
      .then((json) => {
        ...
        const texturePath = self.texturePath && (typeof self.texturePath === "string") ? self.texturePath : THREE.LoaderUtils.extractUrlBase(modelUrl)

        return new THREE.JSONLoader(this.manager).parse(json, texturePath)
      })
      ...
      .then(({
        ...
        self.mesh = new THREE.SkinnedMesh(geometry, materials)
  ...
  bodyPartOpacity(opacity, name) {
    ...
    if (opacity === undefined) {
      return this.mesh.material.map(m => m.name)
    }

    const helpersAndJoints = this.mesh.material.filter(m => typeof m.name === 'string' &&
      (m.name.startsWith('joint') || m.name.startsWith('helper')))

    if (name) {
      parts = this.mesh.material.filter(m => m.name === name)
    } else {
    ...
    return Promise.resolve().then(() => {
      if (this._skinCache[url]) {
        ...
        return new Promise((resolve, reject) => {
          ...
          .catch(err => {
            console.log('failed to create material', err)
          })
      }
    })
      .then((material) => {
        if (this.mesh && this.mesh.material) {
          return this.mesh.material[0] = material
        } else {
          return false
        }
      })
